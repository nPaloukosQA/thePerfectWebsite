...
function buildPString(placeholder, number) {
  ...
  if (number === 1) {
    ...
    newTextNode = document.createTextNode(
      `Here are the first 3 avalible pets: ${petList[0]}, ${petList[1]}, ${petList[2]}`
    ); // could add a for loop to cycle through properties
    newPTag.appendChild(newTextNode);
    placeholder.appendChild(newPTag);
  }
  if (number === 2) {
    newTextNode = document.createTextNode(
      `Some data has been posted! Check the console log to see what.`
    );
    newPTag.appendChild(newTextNode);
    placeholder.appendChild(newPTag);
  }
